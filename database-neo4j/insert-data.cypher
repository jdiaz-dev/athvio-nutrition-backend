...
ON CREATE SET castorOilRec.details = "Consume castor oil.", castorOilRec.isActive = true, castorOilRec.createdAt = datetime(), castorOilRec.updatedAt = datetime()
MERGE (carrotRec:Recommendation {name: "Carrot juice"})
ON CREATE SET carrotRec.details = "Drink 40 ounces of carrot juice.", carrotRec.isActive = true, carrotRec.createdAt = datetime(), carrotRec.updatedAt = datetime()
MERGE (propolisRec:Recommendation {name: "Propolis"})
ON CREATE SET propolisRec.details = "Consume propolis.",  propolisRec.isActive = true, propolisRec.createdAt = datetime(), propolisRec.updatedAt = datetime() 
MERGE (chukrutRec:Recommendation {name: "Chukrut"})
ON CREATE SET chukrutRec.details = "Consume Chukrut.", chukrutRec.isActive = true, chukrutRec.createdAt = datetime(), chukrutRec.updatedAt = datetime() 
MERGE (ketoRec:Recommendation {name: "Keto Diet"})
ON CREATE SET ketoRec.details = "Follow a ketogenic diet.", ketoRec.isActive = true, ketoRec.createdAt = datetime(), ketoRec.updatedAt = datetime() 
MERGE (oreganOilRec:Recommendation {name: "Oregano Oil"})
ON CREATE SET oreganOilRec.details = "Consume oregano oil.", oreganOilRec.isActive = true, oreganOilRec.createdAt = datetime(), oreganOilRec.updatedAt = datetime() 
MERGE (charcoalRec:Recommendation {name: "Heavy Metal Detox"})
ON CREATE SET charcoalRec.details = "Take activated charcoal.", charcoalRec.isActive = true, charcoalRec.createdAt = datetime(), charcoalRec.updatedAt = datetime() 
...
ON CREATE SET gbombs.details = "Include greens, beans, onions, mushrooms, berries and seeds", gbombs.isActive = true, gbombs.createdAt = datetime(), gbombs.updatedAt = datetime() 
